...
class _RenderedHierarchy:
    ...
    def changeDepth(self, chain, newDepth):
        ...
        if _DEBUG['RenderedH'] >= 1.5:  print(
            'change depth of ' + str(chain) + ' from ' + str(oldDepth) + ' to ' + str(newDepth))
        ...
        def _resolveMirror(self, privateMap):
            ...
            for (chain, child) in list(privateMap):
                if _DEBUG['UpdateManager'] >= 3:
                    print("Resolving child " + str(
                        child) + " with status " + child._status + " and special " + child._special)
        ...
        def _updateRecurse(self, chain, style, properties={}, parentMap=None):
            ...
            if _DEBUG['Middle'] >= 3:
                print('_UpdateManager._node._updateRecurse called with\n    ' + '\n    '.join(
                    [str(x) for x in (self, chain, style, properties)]))
    ...
    def update(self, chain, style, properties={}):
        ...
        if _DEBUG['Middle'] >= 1:
            print('\n_UpdateManager.update called with\n    ' + '\n    '.join(
                [str(x) for x in (chain, style, properties)]))
...
class _GraphicsManager:
    ...
    def processCommands(self):
        ...
        try:
            while (
                    _time.time() - start_time) <= MAX_TIME and self._state == 'Running' and not self._commandQueue.empty():
            ...
            if status == 'add':
                ...
                if not isinstance(current._renderedDrawable, _RenderedCanvas) and current._renderedDrawable is not None:
                    ...
                    if node is not None and node._chain[0] == chain[0]:  # TODO: correct treatment of forest???
                        if _DEBUG['Middle'] >= 1: print(
                            'Putting ' + str(current._renderedDrawable) + ' above ' + str(node._renderedDrawable))
                        ...
                        while current != first._prev:
                            if current._renderedDrawable is not None:
                                if below is not None and below._chain[0] == chain[0]:
                                    if _DEBUG['Middle'] >= 1:
                                        print('Putting ' + str(current._renderedDrawable) + ' above ' + str(
...
class Color(object):
    ...
    def _getTkColor(color):
        if color._transparent:
            return ''
        return '#%04X%04X%04X' % (
        256 * color.getColorValue()[0], 256 * color.getColorValue()[1], 256 * color.getColorValue()[2])
...
class Canvas(_GraphicsContainer, _EventTrigger):
    ...
    def zoomView(self, factor, fixedPoint=None):
        ...
        self._transform = self._transform * _Transformation((factor, 0, 0, factor,
                                                             fixedPoint.getX() * (1 - factor),
                                                             fixedPoint.getY() * (1 - factor)))
...
class Text(Drawable):
    ...
    def scale(self, factor):
        ...
        super(Text, self).scale(
            factor)  # transform is really irrelevant, but leaving this to support multiple inheritance between Text and some other shape
        self._size *= factor
        self._update({'font size': self._size})
...
class _RenderedRectangle(_RenderedFillableShape):
    def __init__(self, chain, properties):
        ...
        self._object = self._canvas._canvas.create_polygon(points[0].get(), points[1].get(), points[2].get(),
                                                           points[3].get())
        _graphicsManager._objectIdRegistry[(self._canvas._canvas, self._object)] = self
        _RenderedFillableShape.update(self, properties)
    ...
    def update(self, properties):
        if 'transformation' in properties:
            ...
            for i in range(4):
                points[i] = transform.image(points[i])
            self._canvas._canvas.coords(self._object, points[0].getX(), points[0].getY(), points[1].getX(),
                                        points[1].getY(),
